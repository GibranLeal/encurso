 ...
 *
 * Agregamos prop "collapsed" para saber si solo mostramos iconos.
 */
function MenuNode({ node, level = 0, collapsed = false }) {
  ...
  // Padding por nivel (para jerarquía), pero si está colapsado lo reducimos
  ...
  // Si está colapsado, no queremos “acordeones enormes” abiertos por default.
  // Los cerramos cuando colapsa (para que no se haga un listón largo de iconos).
  useEffect(() => {
    ...
    return (
      <div>
        ...
        >
          ...
          {/* Flecha solo si NO está colapsado (en colapsado estorba) */}
          {!collapsed && (
            <i
              className={`fa-solid ${
        ...
        {/* Submenú: solo se muestra si está abierto y NO está colapsado */}
        {!collapsed && open && (
...
export default function Sidebar() {
  /**
   * 1) Cargamos los módulos desde storage a un state,
   *    para poder refrescar cuando se dispare "encurso:menu-updated".
   */
  const [rawModules, setRawModules] = useState(() => getModulesFromStorage());

  /**
   * 2) Estado "collapsed" (colapsado o expandido).
   *    - Lo guardamos en localStorage para recordar preferencia.
   *    - En móvil lo forzamos a colapsado al cargar.
   */
  ...
  /**
   * 3) Cuando el menú se actualiza desde otra pantalla, refrescamos módulos.
   *    (Tu ModulosPage ya dispara este evento.)
   */
  useEffect(() => {
  ...
  /**
   * 4) Auto-colapsado por breakpoint:
   *    - Si pantalla es "chica", colapsa.
   *    - Si vuelve a grande, NO forzamos expandir (respetamos preferencia del usuario),
   *      pero puedes cambiar eso si quieres.
   *
   * Ajusta el breakpoint a tu gusto:
   *   1024 = lg en Tailwind aprox
   */
  useEffect(() => {
  ...
  /**
   * 5) Persistimos collapsed en localStorage para que se recuerde.
   */
  useEffect(() => {
  ...
  /**
   * 6) Construimos el árbol y los grupos con memo para evitar recalcular en cada render.
   */
  ...
  return (
    <aside
      className={[
        // IMPORTANTE: relative para posicionar la "pestaña" (toggle tab)
        "relative rounded-2xl border border-neutral-200 bg-white shadow-sm",
        // padding interno cambia si está colapsado (más compacto)
        collapsed ? "p-3" : "p-4",
        // ancho + transición suave
        collapsed ? "w-20" : "w-72",
        "transition-all duration-200",
      ].join(" ")}
    >
      {/* ===== Toggle Tab (pestañita) ===== */}
      <button
        type="button"
        ...
        className={[
          ...
          "rounded-r-xl border border-neutral-200 bg-white shadow-sm",
          "hover:bg-neutral-50",
        ].join(" ")}
      ...
      {/* ===== Branding ===== */}
      <div className={["flex items-center gap-3 px-2 py-2"].join(" ")}>
        <img
      ...
      <div className="mt-4 space-y-6">
        {grouped.map(([groupName, items]) => (
          <div key={groupName}>
            {/* Nombre del grupo solo si NO está colapsado */}
            {!collapsed && (
